...
class PolicyTrainerRayProcess(RayProcess):
    def from_pretrained(
        ...
        self.policy: PreTrainedModel = AutoModelForCausalLM.from_pretrained(
            ...
            attn_implementation=model_config.attn_implementation or "flash_attention_2",
            use_cache=False,
        )
...
def main(args: Args, tc: TokenizerConfig, model_config: ModelConfig, reward_fn: Callable):
    ...
    # ray.init(dashboard_host="0.0.0.0", include_dashboard=False, local_mode=True)  # enable debugging from a different machine (e.g., phobos)
    ray.init(dashboard_host="0.0.0.0", include_dashboard=False)  # enable debugging from a different machine (e.g., phobos)
    pg = None
