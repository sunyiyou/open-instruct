...
@dataclass
class Route:
    condition: Optional[str]  # For pullers: 'R', 'B', 'Y', 'G', 'EMPTY'. For painters: None
...
class Robot:
    ...
    def pull_front(self) -> Optional[str]:
        ...
        while iteration < max_iterations:
            ...
            if current_node_id == 'NONE':
                return ExecutionResult(
                    ...
                    path=path,
                    rejection_reason="Routed to NONE"
                )
...
class ManufactoriaParser:
    ...
    def _parse_node(self, lines: List[Tuple[str, int]], start_idx: int) -> int:
        ...
        node_match = re.match(r'^(START|END|PULLER_RB|PULLER_YG|PAINTER_RED|PAINTER_BLUE|PAINTER_YELLOW|PAINTER_GREEN)\s+(\w+):?\s*(?:#.*)?$', line.strip())
    ...
    def _validate_routes(self, node_type: NodeType, routes: List[Route], line_num: int):
        ...
        elif node_type.name.startswith('PAINTER_'):
            if len(routes) != 1:
    ...
    def visualize(self, nodes: Dict[str, Node], save_path: Optional[str] = None, show: bool = True):
...
def visualize_manufactoria_factory(text: str, save_path: Optional[str] = None, show: bool = True):
