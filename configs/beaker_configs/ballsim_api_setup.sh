...
start_uvicorn_servers() {
    echo "Starting Ballsim API servers on $(hostname)"
    ...
    for ((i=0; i<BALLSIM_SERVER_CPUS; i++)); do
        ...
        echo "Starting Ballsim API server on core $CPU_ID, port $PORT"
        
        # Use absolute path and better command structure
        cd "$REPO_PATH"
        
        # Start uvicorn with better error handling
        taskset -c $CPU_ID nohup uvicorn open_instruct.code.ballsim_api:app --host 0.0.0.0 --port $PORT > api_logs/ballsim_server_$PORT.log 2>&1 &
        SERVER_PID=$!
        echo "Ballsim API server started on port $PORT with PID $SERVER_PID"
        ...
        if [ $i -eq 0 ]; then
            ...
            if curl -s http://localhost:$PORT/health > /dev/null; then
                echo "✓ First Ballsim API server is responding on port $PORT"
            else
                echo "⚠ First Ballsim API server not responding on port $PORT, checking logs..."
...
# Main execution
echo "Setting up Ballsim API infrastructure"
...
echo "Testing Ballsim API endpoint..."
...
if curl -s $BALLSIM_API_URL/health > /dev/null; then
    ...
    ps aux | grep nginx
    echo "Sample Ballsim API server logs:"
    ls api_logs/ | head -3 | xargs -I {} tail -3 api_logs/{}
fi

echo "Ballsim API setup complete!"
